...
export default function AppPreview() {
    ...
    useLayoutEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            gsap.from(appPrevieContainerwRef.current, {
                scrollTrigger: {
            ...
            imageRefs.current.forEach((img, index) => {
                ...
                const segmentDuration = 1 / (totalImages - 1); 
                const fadeStart = index * segmentDuration;
                const fadeEnd = fadeStart + segmentDuration * 0.3;
    ...
    return (
        <>
            <section ref={appPreviewRef} id='appPreview'>
                <div ref={appPrevieContainerwRef} className='w-full h-screen flex justify-center items-center bg-amber-100'>
                    {/* ===mobile== */}
                    <div ref={mobileFrameRef} className='w-xs aspect-1/2 rounded-[70px] px-2 py-12 relative flex justify-center items-center '>
                        <img src='/mobileframe.png' alt='mobile frame' className='w-full h-full object-fill absolute top-0 left-0 z-20' />
                        {/* ====mobile inner===== */}
                        <div ref={mobileInnerRef} className='w-full h-full z-10 p-0 bg-amber-300'>
                            {/* ===app content=== */}
